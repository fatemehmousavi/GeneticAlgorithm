...
CostFunction=@(x,X,Y) muta(x,X);     % Cost Function

nVar=5410;            % Number of Decision Variables
...
for i=1:nPop
    ...
    % Evaluation
    pop(i).Cost=CostFunction(pop(i).Position,X);
...
for it=1:MaxIt
    ...
    for k=1:nc/2
        ...
        % Evaluate Offsprings
        popc(k,1).Cost=CostFunction(popc(k,1).Position,X);
        popc(k,2).Cost=CostFunction(popc(k,2).Position,X);
    ...
    for k=1:nm
        ...
        % Evaluate Mutant
        popm(k).Cost=CostFunction(popm(k).Position,X);
